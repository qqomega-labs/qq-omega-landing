...
function createBarrelGeometry(
  ...
  for (let i = 0; i <= bevelSteps; i++) {
    ...
    profile.push(
      new THREE.Vector2(
        radius - bevel + Math.sin(t) * bevel,
        -halfH + bevel - Math.cos(t) * bevel,
      ),
    );
  }
  ...
  for (let i = 0; i <= bevelSteps; i++) {
    ...
    profile.push(
      new THREE.Vector2(
        radius - bevel + Math.cos(t) * bevel,
        halfH - bevel + Math.sin(t) * bevel,
      ),
    );
  }
 ...
 */
function getResponsiveCameraParams(
  w: number,
  h: number,
): { fov: number; y: number; z: number } {
  const aspect = w / h;
...
export function createCoinScene(container: HTMLElement): ICoinScene {
  ...
  const animate = (): void => {
    ...
    if (hasMomentum) {
      ...
      if (
        Math.abs(momentumVelocityX) < 0.0001 &&
        Math.abs(momentumVelocityY) < 0.0001
      ) {
        hasMomentum = false;
        momentumVelocityX = 0;
        momentumVelocityY = 0;
